...
function page(ev,desc,s){
  ...
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${ev.title} | Boombastic</title><meta name="description" content="${desc||ev.description||''}"><link rel="canonical" href="${share}"><script type="application/ld+json">${JSON.stringify(sd)}</script><script type="application/ld+json">${JSON.stringify(bc)}</script></head><body><main style="max-width:980px;margin:20px auto;font-family:system-ui,sans-serif"><h1>${ev.title}</h1><p>${desc||ev.description||''}</p>${ev.start?`<p><strong>Date:</strong> ${new Date(ev.start).toLocaleDateString('en-GB',{year:'numeric',month:'long',day:'numeric'})} <strong>Time:</strong> ${new Date(ev.start).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}${ev.end?`–${new Date(ev.end).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}`:''}</p>`:''}<p><strong>Venue:</strong> ${venue || (ev.location||'')}</p>${ev.bookUrl?`<p><a href="${ev.bookUrl}" target="_blank" rel="noopener">Buy tickets</a></p>`:''}</main></body></html>`;
}
