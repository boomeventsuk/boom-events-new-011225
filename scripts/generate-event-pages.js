/* Generate per-event static pages with clean JSON-LD + a validation report
   Reads: public/events.json (array of events)
...
function pageHtml(ev, desc, slug){
  ...
  const date = ev.start ? new Date(ev.start).toLocaleDateString('en-GB',{year:'numeric',month:'long',day:'numeric'}) : '';
  const st = ev.start ? new Date(ev.start).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}) : '';
  const et = ev.end ? new Date(ev.end ).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}) : '';
...
(function run(){
  ...
  events.forEach(ev => {
    ...
    const html = pageHtml(ev, desc, slug);
    const dir = path.join(outDir, slug);
    ensureDir(dir);
    fs.writeFileSync(path.join(dir,'index.html'), html, 'utf8');

    // quick validation flags for report
    sample.push({
      slug,
      tzOk: /"startDate"\s*:\s*".+?[+\-]\d{2}:\d{2}"/.test(html),
      offersUrlOk: /"offers"\s*:\s*{[^}]*"url"\s*:\s*".+?"/.test(html),
      priceNumeric: /"offers"\s*:\s*{[^}]*"price"\s*:\s*\d+/.test(html)
    });
  });
  ...
  lines.push('');
  lines.push('## Notes');
  lines.push('- Timezone added when missing (+01:00 in Apr–Oct, else +00:00).');
  lines.push('- Non-numeric prices omitted (no "TBA" in JSON-LD).');
  lines.push('- Venue in location.name; city in address.addressLocality.');

  ensureDir(path.join(process.cwd(),'public'));
  fs.writeFileSync(path.join(process.cwd(),'public','geo-seo-validation-report.md'), lines.join('\n'), 'utf8');

  console.log(`Generated ${events.length} event pages. Wrote public/geo-seo-validation-report.md`);
})();