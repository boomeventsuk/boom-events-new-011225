...
// Matches the /event/* SPA fallback line in dist/_redirects. Forced 410
// lines for cancelled and expired event paths are inserted just before it
// so they win Netlify's first-match ordering. The force flag (!) is
// required because a static shell exists at the path and Netlify would
// otherwise serve that shell with a 200.
...
async function writeCancelledEvents(events) {
  ...
  // Same forced-410 treatment the expired shells already get below. Without
  // the force flag (!) Netlify serves the static shell that now exists at
  // /event/{code}/index.html with a 200, which reads to a crawler as a live
  // page. The lines go in before the /event/* SPA fallback so they win
  // Netlify's first-match ordering, and the shell is still the body served,
  // just with an honest 410 status.
  ...
  console.log(`prerender-events: wrote ${cancelled.length} cancelled-event 410 redirects`);
}
...
// public/events-boombastic.json keeps the isCancelled: true entry as the
// internal record. That is what makes cancellation sticky across syncs and
// what the cancelled shells and 410s above are generated from, so it must
// stay in git. The copy Vite has already placed in dist/ is a different
// thing: it is served publicly at /events-boombastic.json, and anyone,
// including an AI crawler or a scraper, can read the full title, date,
// venue, price, Eventbrite id and image of a cancelled event from it. This
// runs after the shells, and after Vite has copied public/ into dist/, and
// removes the cancelled entries from the deployed copy only. Nothing under
// public/ is touched.
const DIST_EVENT_FEEDS = ["events-boombastic.json"];
...
function makeCancelledMatcher(cancelled) {
  const codes = new Set(
    cancelled.map((e) => String(e.eventCode || "").toLowerCase()).filter(Boolean)
  );
  const ids = new Set(
  ...
  return (entry) => {
    ...
    const code = String(entry.eventCode || entry.slug || "").toLowerCase();
...
async function stripCancelledFromDistFeeds(events) {
  const cancelled = events.filter((e) => e.isCancelled === true);
  if (cancelled.length === 0) {
    console.log("prerender-events: stripped 0 cancelled event(s) from dist JSON feeds");
    return;
  }
  const isCancelledEntry = makeCancelledMatcher(cancelled);
...
async function main() {
  ...
  // Deployed machine-readable feeds: a cancelled event must not appear in any
  // publicly served output, so remove it from the dist/ copy last, once the
  // shells that are generated from the full list have all been written.
  await stripCancelledFromDistFeeds(events);
}
